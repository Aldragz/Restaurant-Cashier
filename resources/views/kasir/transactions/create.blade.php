...
/* ITEM GRID TIDAK BOLEH HILANG */
.product-item {
    transition: opacity .15s ease;
}

/* MODE DISABLED (BUKAN display:none) */
.product-item.hidden {
    opacity: 0;
    pointer-events: none;
    position: absolute;
...
<form method="POST" action="/kasir/transactions/store-cart" id="cart-form">
@csrf
...
<div class="mb-3">
    <div class="input-group">
        ...
        <input type="text"
               id="searchInput"
               class="form-control"
               placeholder="Cari produk (makanan / minuman)..."
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <button type="button" class="btn btn-primary w-100 mb-2" onclick="submitCart()">
                    Bayar
                </button>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body text-center">
        ...
        <p class="mb-0">Pesanan berhasil diproses.</p>
      </div>
...
function searchProduct(keyword) {
    ...
    document.querySelectorAll('.product-item').forEach(item => {
        ...
        if (keyword === '') {
            // kembali ke filter kategori
            item.classList.toggle(
                'hidden',
                item.dataset.category !== activeCategory
            );
        } else {
            // search global (tanpa kategori)
            item.classList.toggle(
                'hidden',
...
function submitCart() {
    ...
    fetch('/kasir/transactions/store-cart', {
        method: 'POST',
        headers: {
